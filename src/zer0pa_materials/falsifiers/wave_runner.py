...
5. Records each fired falsifier as a row in ``falsifiers.jsonl`` via the
   hash-chained :class:`~zer0pa_materials.audit.AuditLog`.
...
* :mod:`zer0pa_materials.falsifiers.wave_report` — markdown report generator.
* :mod:`zer0pa_materials.cli.falsification` — CLI subapp.
...
from zer0pa_materials.audit.kg import KGNode, KGNodeType, MaterialsKG
from zer0pa_materials.audit.log import AuditLog
from zer0pa_materials.boundary import RESEARCH_BOUNDARY
from zer0pa_materials.envelope.envelope import Envelope
from zer0pa_materials.envelope.falsifier import FalsifierItem
...
def _route_missing_boundary(fixture_dir: Path) -> list[FalsifierItem]:
    ...
    from zer0pa_materials.boundary import BoundaryError
...
def _route_unstable_phonon(fixture_dir: Path) -> list[FalsifierItem]:
    ...
    from zer0pa_materials.falsifiers.l1_5_falsifiers import dynamical_stability
...
def _route_unreadable_tdb(fixture_dir: Path) -> list[FalsifierItem]:
    ...
    from zer0pa_materials.falsifiers.l3_falsifiers import tdb_parses_in_pycalphad
...
def _route_phasefield_conservation(synth: dict[str, Any]) -> list[FalsifierItem]:
    ...
    from zer0pa_materials.falsifiers.l4_falsifiers import cahn_hilliard_mass_drift
...
def _route_non_spd_tensor(fixture_dir: Path) -> list[FalsifierItem]:
    ...
    from zer0pa_materials.falsifiers.l5_falsifiers import tensor_spd_check
...
class FalsificationWaveRunner:
    ...
    def run_one(
        ...
        except Exception as exc:  # noqa: BLE001 — adversarial run; surface
        ...
        if self.kg is not None:
            ...
            except Exception:  # noqa: BLE001 — KG node uniqueness etc.
                pass
