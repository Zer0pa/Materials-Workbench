...
from zer0pa_materials.adapters.l1 import get_l1_adapter
...
def healthz() -> None:
    ...
    typer.echo(f"QE:               dry-run/parse-only (real: parked)")
    typer.echo(f"CP2K:             dry-run/parse-only (real: parked)")
    typer.echo(f"ABINIT:           dry-run/parse-only (real: parked)")
