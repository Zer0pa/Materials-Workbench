...
1. ``ZER0PA_MATERIALS_WORKBENCH_REPO_ROOT`` environment variable.
2. Walk up from this module's file path looking for a ``pyproject.toml``
   whose ``[project].name`` is ``zer0pa-materials-workbench``.
...
_PYPROJECT_NEEDLE: Final[str] = 'name = "zer0pa-materials-workbench"'
...
def repo_root() -> Path:
    """Return the absolute path to the zer0pa-materials-workbench repo root.
    ...
    if env:
        ...
        raise RepoRootNotFoundError(
            ...
            "zer0pa-materials-workbench repo (no pyproject.toml with that name)."
        )
    ...
    if found is not None:
        return found
    raise RepoRootNotFoundError(
        "Could not locate zer0pa-materials-workbench repo root. Set "
        "ZER0PA_MATERIALS_WORKBENCH_REPO_ROOT or run from inside the repo."
    )
