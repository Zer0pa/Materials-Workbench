...
:class:`~zer0pa_materials_workbench.envelope.config.MaterialsConfig` and decides
which physical backend serves a given layer/endpoint:

* ``runpod_rest``   → :class:`~zer0pa_materials_workbench.runpod.rest_client.RunpodRestClient`
                      (real HTTP), provided credentials are present.
* ``runpod_mock``   → :func:`~zer0pa_materials_workbench.runpod.mock_backends.build_runpod_mock_envelope`
...
2.  When the layer's ``*_BACKEND`` is ``runpod_rest`` AND credentials are
    ...
    appends a :class:`~zer0pa_materials_workbench.audit.BlockedSourceManifest` to
